...
export default function TokenOrderForm({ sheet, sheetDeals, retailerId, retailerName }: {
  sheet: any
  sheetDeals: any[]
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const lines = sheetDeals
      ...
      .map(sd => ({ deal_id: sd.deal_id, alloc_qty: parseInt(qtys[sd.id], 10) }))
  ...
  if (submitted) {
    return (
      <div className="bg-white border border-zinc-100 rounded-2xl p-10 text-center">
        ...
        <p className="text-sm text-zinc-400">Our team will be in touch to confirm your order. It will ship with your next available delivery.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Store identity — read-only, pre-identified from token */}
      <div className="bg-white border border-zinc-100 rounded-2xl p-6 space-y-4">
        ...
        <div className="flex items-center gap-3 bg-zinc-50 border border-zinc-100 rounded-lg px-4 py-3">
          <Store size={14} className="text-zinc-400 flex-shrink-0" />
          <span className="text-sm font-medium text-zinc-900">{retailerName || "Your store"}</span>
        </div>
